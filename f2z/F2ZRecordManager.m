...
- (void) storeToDatabase:(F2ZRecord*)record
{
    ...
    BOOL rc = [db executeUpdate:@"update history set data=? where scn=? and day=? and month=? and year=?",
               data,
               [NSNumber numberWithInteger:[record scn]],
               [NSNumber numberWithUnsignedChar:[record day]],
               [NSNumber numberWithUnsignedChar:[record month]],
               [NSNumber numberWithUnsignedChar:[record year]]];
    if (rc != 1) {
        [NSException raise:@"DatabaseException"
                    format:@"failure in updating swiped record:%@", record];
    }
